...
describes what users get. Pick a format with `-o`, or let thegioididong choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
thegioididong <command> -o table   # aligned columns for reading
thegioididong <command> -o jsonl   # one JSON object per line, for piping
thegioididong <command> -o json    # a single JSON array
thegioididong <command> -o csv     # spreadsheet friendly
thegioididong <command> -o tsv     # tab-separated
thegioididong <command> -o url     # just the URL column
thegioididong <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
thegioididong <command> --fields id,title,url
```
...
```bash
thegioididong <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
thegioididong <command>            # a table, because this is a terminal
thegioididong <command> | wc -l    # JSONL, because this is a pipe
```
